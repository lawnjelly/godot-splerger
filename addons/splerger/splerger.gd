...
func _find_meshes_recursive(node : Node, meshlist, si : _SplitInfo):
    ...
    if node is MeshInstance3D:
        ...
        if (splits_x + splits_y + splits_z) > 3:
            ...
            prints("\tfound mesh to split : ", mi.get_name())
            prints("\t\tsplits_x : ",  str(splits_x), " _y " , str(splits_y), " _z " , str(splits_z))
...
func split(mesh_instance : MeshInstance3D, attachment_node : Node, grid_size : float, grid_size_y : float, use_local_space : bool = false, delete_orig : bool = false):
    ...
    prints(mesh_instance.get_name(), " : x_splits ", str(si.x_splits), " y_splits " , str(si.y_splits), " z_splits ",str(si.z_splits))
...
func _split_mesh(mdt : MeshDataTool, orig_mi : MeshInstance3D, grid_x : float, grid_y : float, grid_z : float, si : _SplitInfo, attachment_node : Node, faces_assigned, world_verts):
...
#	var sz = ""
    ...
    new_mi.set_name(str(orig_mi.get_name()) + "_" + str(grid_x) + str(grid_z))
...
func split_by_surface(orig_mi : MeshInstance3D, attachment_node : Node, use_local_space : bool = false):
    ...
    print ("split_by_surface " + str(orig_mi.get_name()))
...
func merge_suitable_meshes_across_branches(root : Position3D):
    var master_list = []
...
func _find_suitable_meshes(child_list, node : Node):
    ...
    if node is MeshInstance3D:
        ...
        if mi.get_surface_material_count() <= 1:
            print("found mesh instance " + str(mi.get_name()))
...
func merge_meshinstances(mesh_array, attachment_node : Node, use_local_space : bool = false, delete_originals : bool = true):
    ...
    var sz = str(first_mi.get_name()) + "_merged"
    new_mi.set_name(sz)
...
func _merge_meshinstance(st : SurfaceTool, mi : MeshInstance3D, use_local_space : bool, vertex_count : int):
    ...
    print("merging meshinstance : " + str(mi.get_name()))
    var mesh = mi.mesh
...
func _split_mesh_by_surface(mdt : MeshDataTool, orig_mi : MeshInstance3D, attachment_node : Node, surf_id : int, use_local_space : bool):
    ...
    var sz = str(orig_mi.get_name()) + "_" + str(surf_id)
    if mat:
